...
# ABOUT 75% of this code was obtained from (https://www.youtube.com/watch?v=47c_1wOa2so&list=WL&index=11&t=1069s)
...
# I learned a lot of things about pygame. The code I sourced this from is written very cleanly and helped me undesrtand pygame easily.
...
KEY_COLOR = (245, rand_color, 245)#MA
...
def draw_piano(whites, blacks):
    #rand_color = randint(230,254) #MA #EPILEPSY ALERT
    ...
    for i in range(52):
        ...
        screen.blit(key_label, (i * 35 + 3, HEIGHT - 20)) #Writing these fonts on the keys
    skip_count = 0
    last_skip = 2
    skip_track = 2
    black_rects = []
...
def draw_title_bar(): #draws the different lines throughout
    ...
    screen.blit(scale_text, (910, 10)) #MA
    chord_text = nati2_font.render('W plus any key plays the seven chords of the minor key', True, 'white') #MA
    screen.blit(chord_text, (910, 50)) #MA
    chord_text = nati2_font.render('E plus any key plays the a 2-five-1 progression in the major key', True, 'white') #MA
    screen.blit(chord_text, (910, 80)) #MA
    pygame.draw.rect(screen, 'black', [109, 0, 790, 100]) #MA
    key_played = nati3_font.render(scale_string, True, 'white') #MA
    screen.blit(key_played, (115, 40)) #MA
...
def minor_scale_mover(NOTEEE): #plays a minor scale #MA
    ...
    for increment in range(8):
        ...
        if NOTEEE[1] != '#' and scaler < 2: 
            ...
            #green_draw (active_whites, active_blacks)
            #print(active_whites, active_blacks)
        elif NOTEEE[1] == '#' and scaler < 2: #and 'D' not in NOTEEE and 'A' not in NOTEEE:
            index = piano_labels.index(NOTEEE)
            scale_track += 1
            scaler += 1
            NOTE_KEY[NOTEEE] = SCALE_DEGREE
            SCALE_DEGREE += 1
            piano_sounds[index].play()
            second_index = black_labels.index(NOTEEE)
            ...
            #green_draw (active_whites, active_blacks)
            #print(active_whites, active_blacks)
        time.sleep(delay) #delay between playing notes in the for loop

    #appends identity of keys to dictionary "NOTE_KEY" for printing later
    global scale_string
    scale_string = "The notes played along with their scale degrees: " 
    for i in NOTE_KEY:
        scale_string += i + '--' + str(NOTE_KEY[i]) + ' '
    return scale_string

def chord_player (NOTEEE): #MA
    ...
    for increment in range(7):
        ...
        # this part could be shorter but it is much clearer this way I think for someone that does not know scale degrees
    ...
    # Similar to whats done in the previous function but has a little more work on the pattern
    global scale_string
    scale_string = "The chords played along with their scale degrees: " 
    for i in NOTE_KEY:
        if (NOTE_KEY[i] == 1 or NOTE_KEY[i] == 5 or NOTE_KEY[i] == 4) and i[1] != '#':
            scale_string += i[0] + 'MIN' + "--" + str(NOTE_KEY[i]) + ' '
        elif (NOTE_KEY[i] == 7 or NOTE_KEY[i] == 6 or NOTE_KEY[i] == 3 or NOTE_KEY[i] == 8) and i[1] != '#':
            scale_string += i[0] + 'MAJ' + "--" + str(NOTE_KEY[i]) + ' '
        elif NOTE_KEY[i] == 2 and i[1] != '#':
            scale_string += i[0] + 'DIM' + "--" + str(NOTE_KEY[i]) + ' '
        elif (NOTE_KEY[i] == 1 or NOTE_KEY[i] == 5 or NOTE_KEY[i] == 4) and i[1] == '#':
            scale_string += i[0] + i[1] + 'MIN' + "--" + str(NOTE_KEY[i]) + ' '
        elif (NOTE_KEY[i] == 7 or NOTE_KEY[i] == 6 or NOTE_KEY[i] == 3 or NOTE_KEY[i] == 8) and i[1] == '#':
            scale_string += i[0] + i[1] + 'MAJ' + "--" + str(NOTE_KEY[i]) + ' '
        elif NOTE_KEY[i] == 2 and i[1] == '#':
            scale_string += i[0] + i[1] + 'DIM' + "--" + str(NOTE_KEY[i]) + ' '
